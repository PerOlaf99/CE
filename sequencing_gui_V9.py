...
class SequencingGUI(QMainWindow):
    ...
    # (label1, range1, label2, range2) per smoothing method
    _SMOOTH_PARAM_CONFIG = {
    ...
    def _on_smooth_method_changed(self, method):
        ...
        label1, range1, label2, range2 = self._SMOOTH_PARAM_CONFIG.get(
    ...
    # (label, range) per baseline method
    _BASELINE_PARAM_CONFIG = {
    ...
    def _on_baseline_method_changed(self, method):
        label, rng = self._BASELINE_PARAM_CONFIG.get(method, ('Window:', (20, 1000)))
    ...
    def _airpls_baseline(self, y, lam, itermax=15):
        """Adaptive iteratively reweighted penalized least squares (Zhang et
        al. 2010). Like ALS, but the asymmetry weights are re-derived every
    ...
    def _snip_baseline(self, y, iterations):
        """SNIP (Statistics-sensitive Non-linear Iterative Peak-clipping),
        widely used in spectroscopy/chromatography baseline removal. Works on
        an LLS-transformed copy of the trace (compresses peak heights so tall
        peaks don't dominate the clipping) and iteratively clips each point
        down to the average of its two neighbors at growing distance,
        whenever that average is lower - carving out everything but the
        underlying baseline."""
        ...
        for p in range(1, iterations + 1):
            left = self._shift_channel(v, p)
            right = self._shift_channel(v, -p)
            v = np.minimum(v, 0.5 * (left + right))
        baseline = (np.exp(np.exp(v) - 1) - 1) ** 2 - 1
        return np.clip(baseline, 0, None)

    def _wavelet_denoise(self, y, pywt, level, threshold_scale=1.0, wavelet='db4'):
        """Wavelet-domain denoising: decompose, soft-threshold the detail
        coefficients using a noise estimate from the finest scale (median
        absolute deviation), then reconstruct. Edge-preserving compared to
        the other smoothers here - it can knock down noise without rounding
        off sharp peak tops the way a wide moving-average or Gaussian would."""
    ...
    def _process(self):
        ...
        # Mobility shift: each dye channel can migrate at a slightly different
        # rate, so let the user nudge each channel independently (in scans).
        # Positive shift delays the channel (moves its peaks right); negative
        # advances it (moves peaks left). Edge-padded, not wrapped.
        for ch in range(4):
            shift = self.mobility_spins[ch].value()
            if shift != 0:
                raw[:, ch] = self._shift_channel(raw[:, ch], shift)
        ...
        if bl_method == 'Rolling Minimum':
            ...
            lam = bw
            for ch in range(4):
                bl[:, ch] = self._airpls_baseline(raw[:, ch], lam)
        elif bl_method == 'SNIP':
            for ch in range(4):
                bl[:, ch] = self._snip_baseline(raw[:, ch], bw)
        elif bl_method == 'Morphological (Top-hat)':
            from scipy.ndimage import grey_opening
            size = max(3, bw)
            for ch in range(4):
                bl[:, ch] = grey_opening(raw[:, ch], size=size)
        elif bl_method == 'Polynomial Detrend':
            order = max(1, min(bw, len(raw) - 1))
            x_idx = np.arange(len(raw), dtype=np.float64)
            for ch in range(4):
        ...
        elif self._smooth_mode == 'Butterworth':
            from scipy.signal import butter, filtfilt
            order = max(1, min(so, 10))
        ...
        elif self._smooth_mode == 'Wavelet':
            try:
                import pywt
            except ImportError:
                self.status.setText('Wavelet smoothing needs PyWavelets: pip install PyWavelets')
            else:
                level = max(1, min(sw, 8))
                thresh_scale = max(so, 1) / 100.0
                for ch in range(4):
                    sm[:, ch] = self._wavelet_denoise(sm[:, ch], pywt, level, thresh_scale)
        elif self._smooth_mode == 'LOWESS':
            try:
                from statsmodels.nonparametric.smoothers_lowess import lowess as sm_lowess
            except ImportError:
                self.status.setText('LOWESS needs statsmodels: pip install statsmodels')
            else:
        ...
        # Matrix separation
        mix = self._get_matrix()
        try:
            inv = np.linalg.inv(mix)
        except np.linalg.LinAlgError:
            inv = np.linalg.pinv(mix)
