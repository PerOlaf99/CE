...
def _rubberband_baseline(y, smooth_win=1):
    ...
    to form a baseline floor.  Works well when the baseline is a smooth,
    monotonically varying background.  smooth_win (if > 1) pre-smooths
    the signal via a moving average before hull construction to suppress
    noise that would otherwise create spurious hull vertices."""
    from scipy.ndimage import uniform_filter1d
    n = len(y)
    s = min(int(smooth_win), n) if smooth_win > 1 else 1
    if s > 1:
        y = uniform_filter1d(y.astype(np.float64), size=s, mode='nearest')
    else:
...
def _asyls_baseline(y, lam, p=0.01, niter=10):
    """Asymmetric Least Squares baseline (Eilers 2001).  Same penalty as ALS
    but uses a constant asymmetry parameter p instead of iterating the
    weights; slightly faster and more predictable than ALS for a fixed p."""
    from scipy import sparse
    from scipy.sparse.linalg import spsolve
    n = len(y)
    y = y.astype(np.float64)
...
def _arpls_baseline(y, lam=1e5, max_iter=100, tol=1e-5):
    """Adaptive iteratively reweighted PLS baseline (Oller-More et al.
    2006).  Uses a smooth weight function based on the standard deviation of
    the positive residual, giving robust automatic weight updates without
    the manual p parameter of ALS.  Generally converges to a better baseline
    than airPLS for noisy CE data."""
    from scipy import sparse
    from scipy.sparse.linalg import spsolve
    n = len(y)
    y = y.astype(np.float64)
...
def compute_baseline(raw, method, window, window2=None):
    """raw: (n,4). window: primary tunable param for `method`.
    window2: optional secondary param (asymmetry p*100 for ALS/AsyLS,
    max iterations for airPLS/arPLS). Ignored if None or method has no
    secondary param."""
    ...
    bw = window
    bw2 = window2 if window2 is not None else 0
    if method == 'None':
        return bl
    if method == 'Rolling Minimum':
        ...
        lam = bw
        p = float(bw2) / 100.0 if bw2 else 0.005
        e = np.ones(n)
        D2 = sparse.diags([e, -2 * e, e], [0, 1, 2], shape=(n - 2, n))
        A = lam * D2.T @ D2
        for ch in range(4):
        ...
        itermax = max(5, min(int(bw2) if bw2 else 15, 50))
        for ch in range(4):
            bl[:, ch] = _airpls_baseline(raw[:, ch], bw, itermax=itermax)
    elif method == 'SNIP':
        for ch in range(4):
        ...
        for ch in range(4):
            bl[:, ch] = _rubberband_baseline(raw[:, ch], smooth_win=max(1, int(bw)))
    elif method == 'AsyLS':
        lam = bw
        p = float(bw2) / 100.0 if bw2 else 0.01
        for ch in range(4):
            bl[:, ch] = _asyls_baseline(raw[:, ch], lam, p=p, niter=10)
    elif method == 'arPLS':
        lam = bw
        max_iter = max(5, min(int(bw2) if bw2 else 100, 200))
        for ch in range(4):
            bl[:, ch] = _arpls_baseline(raw[:, ch], lam=lam, max_iter=max_iter)
    else:
...
def separate_channels(sm, bl, matrix):
    ...
    # Baseline method 'None' yields an all-zero baseline; the per-channel
    # dye-gain normalization then has nothing to divide by, so fall back to
...
def full_pipeline(raw, mobility_shifts, baseline_method, baseline_window,
    ...
    ``matrix_apply_point`` selects which stage the crosstalk (dye-bleed)
    ...
    """
    raw = raw.copy()
    bl = compute_baseline(raw, baseline_method, baseline_window, baseline_window2)
    corr = np.clip(raw - bl, 0, None)
    sm = smooth_signal(corr, smooth_method, smooth_window, smooth_order)
    if matrix_apply_point == 'raw':
        sep_raw = separate_channels(raw, bl, matrix)
        sep_bl = compute_baseline(sep_raw, baseline_method, baseline_window,
                                  baseline_window2)
        sep_corr = np.clip(sep_raw - sep_bl, 0, None)
        separated = smooth_signal(sep_corr, smooth_method, smooth_window,
                                  smooth_order)
    elif matrix_apply_point == 'corrected':
        sep_corr = separate_channels(corr, bl, matrix)
        separated = smooth_signal(sep_corr, smooth_method, smooth_window,
                                  smooth_order)
