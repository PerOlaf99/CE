...
report("baseline", {})
for dr in (0, 1):
    report(f"a_dominance_radius={dr}", dict(a_dominance_radius=dr))
for dr in (0, 2):
    report(f"t_dominance_radius={dr}", dict(t_dominance_radius=dr))
for dm in (0.85, 0.95):
    report(f"dominance={dm}", dict(dominance=dm))
for dr in (0, 1):
    for dm in (0.85,):
        report(f"a_dr={dr},dom={dm}", dict(a_dominance_radius=dr, dominance=dm))
