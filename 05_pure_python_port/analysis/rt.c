...
void four1(double data[], unsigned long nn, int isign){
  ...
  while(n>mmax){ istep=mmax<<1; theta=isign*(2.0*NR_PI/mmax);
    ...
    for(m=1;m<mmax;m+=2){ for(i=m;i<=n;i+=istep){ j=i+mmax;
      ...
      wr=(wtemp=wr)*wpr-wi*wpi; wi=wi*wpr+wtemp*wpi+wi; }
    mmax=istep; }
}
int main(void){
  double d[33]; size_t i; unsigned long nn;
  for(nn=2;nn<=8;nn+=2){
    for(i=1;i<=2*nn;i++) d[i]=((double)rand()/RAND_MAX);
    double orig[33]; for(i=1;i<=2*nn;i++) orig[i]=d[i];
    four1(d,nn,1); four1(d,nn,-1);
    double mx=0; for(i=1;i<=2*nn;i++){ double e=fabs(d[i]-orig[i]*nn); if(e>mx) mx=e; }
    printf("nn=%2lu roundtrip maxerr=%.3e\n", nn, mx);
  }
  return 0;
}
