"""2-D sweep of Wiener deconvolution width x regularization (sigma_scale x noise_reg).

Maps the Pareto frontier between matched bases (GOLDEN primary) and the
longest-error-free-run / substitution bar. Seeks a config that keeps matched
~816 and bit >= DLL while cutting substitutions and lifting the longest run.

Usage:
    BLAST_DIR=/path/to/ncbi-blast-*/bin python exp_deconv2.py
"""
import glob
import json
import os
...
CONFIGS = {}
for _s in (1.05, 1.08, 1.10, 1.12):
    for _r in (0.06, 0.07, 0.08):
        CONFIGS["s%.2f_r%.2f" % (_s, _r)] = P(gaussian_recon_sigma_scale=_s,
                                              gaussian_recon_noise_reg=_r)
CONFIGS["s1.10_r0.065"] = P(gaussian_recon_sigma_scale=1.10, gaussian_recon_noise_reg=0.065)
CONFIGS["s1.10_r0.075"] = P(gaussian_recon_sigma_scale=1.10, gaussian_recon_noise_reg=0.075)
...
def main():
    ...
    for name, cfg in CONFIGS.items():
        fa = os.path.join(be.WORK, "dev3_%s.fasta" % name)
        ...
        out[name] = agg
        print("%-14s matched/w=%.1f bits/w=%.1f %%ID=%.2f longN=%.1f gaps=%-5d ins=%-5d mismatch=%-5d rlen=%.0f canon=%.2f"
              % (name, matched, bits, agg["mean_pident"], agg["mean_longest_perfect"],
                 gaps, ins, mismatch, rlen, agg["perbase_vs_ref"]), flush=True)
    json.dump({"results": out}, open(os.path.join(_HERE, "report_deconv3.json"), "w"),
              indent=2, default=str)
    print("wrote report_deconv3.json")
