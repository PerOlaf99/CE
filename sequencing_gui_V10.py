...
def dsp_compute_baseline(raw, method, window):
    ...
    if method == 'Rolling Minimum':
        ...
        for ch in range(4):
            bl[:, ch] = dsp_airpls_baseline(raw[:, ch], bw)
    elif method == 'SNIP':
        for ch in range(4):
        ...
        lam = bw
        for ch in range(4):
            bl[:, ch] = dsp_arpls_baseline(raw[:, ch], lam=lam, max_iter=100)
    else:
...
def pc_call_bases_with_shifts(separated, shifts, min_distance=6,
    ...
    *after* baseline/smoothing/matrix, then runs ``find_peaks`` independently
    ...
    channels = []
    for ch in range(4):
        shifted = dsp_shift_channel(separated[:, ch], int(shifts[ch]))
        if normalize:
        ...
        scale = np.percentile(np.clip(shifted, 0, None), 99.5)
...
class SequencingGUI(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        blg2.addWidget(self.bl2_param_label)
        blg2.addWidget(self.bl2_slider)
        blg2.addWidget(self.bl2_spin)
        blg_g.addLayout(blg2)
        ...
        self.distance_spin.setToolTip('Minimum horizontal distance between detected peaks (scans)')
        ...
        self.prominence_spin.setToolTip('Minimum prominence (0.001 = 0.1% of channel max)')
    ...
    def _update_plot(self):
        ...
        # Plot 3: Separated (with mobility shifts applied) vs ESD
        ...
        for ch in range(4):
            ax3.plot(x_esd_aligned, esd_plot[:, ch], color=CHAN_COLORS[ch],
        ...
        if peaks is not None:
            ...
            for p in peaks:
                ...
                if 0 <= x_disp < len(separated_shifted) and sep_max[dom_ch] > 0:
                    y_disp = separated_shifted[x_disp, dom_ch] + 0.05 * sep_max[dom_ch]
                    ax3.text(x_disp, y_disp, base, fontsize=6, ha='center',
                             va='bottom', color=color, clip_on=True,
                             fontweight='bold')
                else:
                    ax3.text(x_disp, 0.94, base,
        ...
        if peaks is not None:
            ...
            for p in peaks:
                ...
                ax4.text(x_disp, y_disp, base, fontsize=6, ha='center',
                         va='bottom', color=color, clip_on=True,
                         fontweight='bold')
        ...
        # ESD match percentage (samples separated trace at ESD's own positions)
        ...
        # Independent peak-calling on shifted separated trace with IUPAC labels
        if self.rsd_raw is not None:
            try:
                pos, iupac_seq, base_groups, intens = pc_call_bases_with_shifts(
                    separated_shifted, shifts,
                    min_distance=max(1, self.distance_spin.value()),
                    prominence_frac=self.prominence_spin.value() / 1000.0,
                    tolerance=4,
                )
                ...
                for p, letter in zip(pos, iupac_seq):
                    dom_ch = np.argmax(separated_shifted[p]) if len(separated_shifted) > p else -1
                    if 0 <= p < len(separated_shifted) and sep_max[dom_ch] > 0:
                        y_disp = separated_shifted[p, dom_ch] + 0.08 * sep_max[dom_ch]
                        ax3.text(p, y_disp, letter, fontsize=5, ha='center',
                                 va='bottom', color='black', clip_on=True,
                                 fontweight='bold',
                                 bbox=dict(facecolor='yellow', alpha=0.5, pad=0.3))
                if iupac_seq and len(iupac_seq) > 10:
                    ...
                    ax3.text(0.99, 0.07, f'Independent: {len(iupac_seq)} bases ({pct_manual:.1f}% vs ESD)',
