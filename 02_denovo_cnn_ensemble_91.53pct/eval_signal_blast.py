#!/usr/bin/env python3
"""eval_signal_blast.py - FULL DE-NOVO with ML true-signal region.

Candidate detection is restricted to the ML-predicted true-signal window
[sig_start, sig_stop] predicted by signal_mask_model from RAW signal + current.
Nothing outside the region is even detected - i.e. we never basecall the
background (primer front / dye blob / dead tail).

Pipeline: raw+current -> mask model -> (start,stop) -> dll_peaks(region=...)
-> v6 CNN labels -> BLAST vs M13.
"""
...
def call_well(well, sig_model, models, cuts):
    raw = parse_rsd(os.path.join(PLATE, well + '.rsd'))[CH].values.astype(np.float64)
    cur = np.load(os.path.join(PLATE, well + '.rsd'))['Current'].astype(float) \
        if False else parse_rsd(os.path.join(PLATE, well + '.rsd'))['Current'].values.astype(float)
    s0, s1 = predict_region(sig_model, raw, cur)
    ...
    return dict(read=read, n=len(read), npos=len(pos)), (s0, s1, len(pos))
...
def main():
    ...
    for well in wells:
        ...
        res, info = call_well(well, sig, models, cuts)
        if res is None:
            rows.append((well, dllr['matched'] if dllr else 0, 0, 0, 0.0, *info))
            continue
        b = blast_eval(res['read'])
        matched = b['matched'] if b else 0
        ident = b['identity'] if b else 0.0
        rows.append((well, dllr['matched'] if dllr else 0, matched, res['n'], ident, *info))
    ...
    for (w, dm, om, nb, idn, s0, s1, nc) in rows:
        print(f'  {w} DLL {dm:4d} ours {om:4d} ({om-dm:+d}) nb {nb} '
              f'region[{s0}-{s1}] cand {nc}')
