#!/usr/bin/env python3
"""eval_v8_esdpos.py - CEILING for v8 corrector: DLL positions +
(v6 CNN probs + bandstats) -> v8 corrector labels -> BLAST.  Directly tests
whether DSP per-band evidence fixes the CNN's remaining column errors."""
import os, sys
import numpy as np
...
def main():
    prefix = sys.argv[1] if len(sys.argv) > 1 else 'base_caller_model_v8_corr'
    nreg = 8
    v6 = [tf.keras.models.load_model(f'base_caller_model_v6_cal8_r{r}.keras',
                                     compile=False) for r in range(nreg)]
    v8 = [tf.keras.models.load_model(f'{prefix}_r{r}.keras', compile=False)
          for r in range(nreg)]
    # same stats as train_corr (computed on train subset of corr_training)
    ...
    for well in wells:
        ...
        ok = (pp >= 20) & (pp < len(sep) - 20) & np.isin(list(E['sequence']),
                                                         list('ACGT'))
        pp = pp[ok]
        if len(pp) < 50:
            m6.append(0); m8.append(0); m_dll.append(0); continue
        ...
        read6 = ''.join(LABELS[i] for i in P[:, :4].argmax(1))
    ...
    print(f'wells={len(wells)}  DLL-pos+v8(mlp)={np.mean(m8):.1f}  '
          f'DLL-pos+v6(cnn)={np.mean(m6):.1f}  DLL={np.mean(m_dll):.1f}')
    print(' by-well (DLL v8 v6):')
    for w, a, b, c in zip(wells, m_dll, m8, m6):
        print(f'  {w}: {a} {b} {c}')
