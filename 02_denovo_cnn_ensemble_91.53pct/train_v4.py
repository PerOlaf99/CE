...
def main():
    ...
    ap.add_argument('--npz', default='v3_training.npz')
    ap.add_argument('--prefix', default='base_caller_model_v3_')
    args = ap.parse_args()
    npz_name = args.npz
    PREFIX = args.prefix
    d = np.load(os.path.join(HERE, npz_name), allow_pickle=True)
    ...
    for r in range(4):
        ...
        for ep in range(EPOCHS):
            ...
            if acc > best[0]:
                best = (acc, ep)
                model.save(os.path.join(HERE, PREFIX + f'{REGIONS[r]}.keras'))
