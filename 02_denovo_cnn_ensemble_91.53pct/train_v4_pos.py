#!/usr/bin/env python3
"""train_v4_pos.py - train CNN with reference-cleaned labels + position channel.

Combines two improvements:
  1. M13 reference labels (not DLL/ESD) from m13_clean_training.npz
  2. 5th input channel: normalized scan position (0=start, 1=end of read)
     so the model learns signal decay at the tail.

The 5th channel is appended after z-scoring the 4 raw channels:
  X shape = (N, 31, 5) where channel 4 = linearly interpolated position.

Output: base_caller_model_v4_pos.keras
"""
import os, sys
import numpy as np
...
def add_position_channel(X4, scans, all_wells, well_ids):
    """Append a 5th channel: normalized position within each well's read.

    X4: (N, 31, 4) raw features
    scans: (N,) scan positions
    all_wells: list of unique well IDs
    well_ids: (N,) per-sample well ID

    Returns: (N, 31, 5) with channel 4 = normalized position [0, 1]
    """
    N = len(X4)
    ...
    for w in all_wells:
        ...
        pos_frac = (w_scans - s_min) / s_range  # [0, 1]
        # Broadcast to 31 time points
...
def main():
    ...
    print(f'wells: {len(all_wells)}  train: {tr.sum()}  test: {te.sum()} '
          f'({len(test_wells)} held-out wells)')
    ...
    # Z-score the 4 raw channels, keep position channel as-is
    ...
    for ep in range(25):
        ...
        if acc > best_acc:
            ...
            model.save(os.path.join(HERE, 'base_caller_model_v4_pos.keras'))
        if ep == 1 and best_acc < 0.6:
            print('aborting: not converging')
            break
    print(f'BEST val_acc={best_acc:.4f} @epoch {best_ep + 1}')
    print(f'saved: {os.path.join(HERE, "base_caller_model_v4_pos.keras")}')
