#!/usr/bin/env python3
"""build_corr_v9.py - corrector training features for the v9 CNNs.

Same schema as corr_training.npz (v8), but CNN probs come from the v9
adaptive-width per-region models (W_R), so retraining the second-stage MLP
with matching features is valid:
  X[i] = concat(p[0:4] v9 probs, f[0:6] bandstat) -> 10 floats
  y    = esd base 0-3
  region, split, well, scan.
Output: corr_training_v9.npz
"""
...
def main():
    ...
    v9 = [tf.keras.models.load_model(f'{PREFIX}_r{r}.keras', compile=False)
          for r in range(8)]
    ...
    for k, wname in enumerate(wells):
        ...
        ok = (pp >= 20) & (pp < n - 20) & np.isin(seq, list(BASE_MAP))
        pp, seq = pp[ok], seq[ok]
        if len(pp) < 50:
            continue
        ...
        for r in range(8):
            ...
            if idx:
                W = W_R[r]
                Xc = np.array([window(sep, int(pp[i]), W, n) for i in idx])
                P[idx] = v9[r].predict(zscore(Xc), batch_size=256, verbose=0)
        ...
        region.append(regs)
        split.append(np.full(nP, wname in tr, bool))
        well.append(np.full(nP, wname, object))
        scan.append(pp)
        if (k + 1) % 12 == 0:
    ...
    split = np.concatenate(split); well = np.concatenate(well)
    scan = np.concatenate(scan)
    np.savez_compressed(os.path.join(HERE, 'corr_training_v9.npz'),
                        X=X, p=p, f=f, y=y, region=region, split=split,
                        well=well, scan=scan)
    print(f'saved corr_training_v9.npz: X={X.shape} tr={split.mean():.3f} '
