#!/usr/bin/env python3
"""train_v4_clean.py - train CNN on reference-cleaned M13 labels.

Uses m13_clean_training.npz (extracted by extract_m13_clean_training.py)
which labels windows with TRUE M13 reference bases instead of DLL/ESD calls.
Drops DLL insertion errors, relabels DLL mismatches to correct M13 base.

This should produce a better model because ~9.5% of ESD labels were wrong
(7,780 corrections out of 82,047 samples).
"""
import os, sys
import numpy as np
...
def main():
    ...
    print(f'classes: {dict(enumerate(np.bincount(y, minlength=4).tolist()))}')
    ...
    for ep in range(25):
        ...
        if acc > best_acc:
            ...
            model.save(os.path.join(HERE, 'base_caller_model_v4_clean.keras'))
        if ep == 1 and best_acc < 0.6:
            print('aborting: not converging')
            break
    print(f'BEST val_acc={best_acc:.4f} @epoch {best_ep + 1}')
    print(f'saved: {os.path.join(HERE, "base_caller_model_v4_clean.keras")}')
