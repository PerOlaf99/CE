#!/usr/bin/env python3
"""train_v5raw.py - velocity-featured CNN on the RAW-derived book.

Dual-branch:
  window: 31x4 raw adaptive window (conv stack, as v3/v4)
  aux   : (fwhm_norm, spacing_norm, scan_frac, current_norm) Dense torso
Merged -> 4-class softmax. Book=train wells, TEST=held-out wells.
Saves best model as base_caller_model_v5raw.keras (2-input).
"""
import os, sys
import numpy as np
...
def main():
    ...
    ap.add_argument('--npz', default='v5raw_book.npz')
    ap.add_argument('--book', default=None)
    ap.add_argument('--model', default='base_caller_model_v5raw.keras')
    args = ap.parse_args()
    ...
    te = ~trm
    print(f'book wells {len(book)}  train {trm.sum()}  test {te.sum()}')
    ...
    def model_fn():
        ...
        aux_in = tf.keras.Input(shape=(aux.shape[1],))
