...
from scipy.signal import savgol_filter, correlate as sig_correlate
...
class SequencingGUI(QMainWindow):
    ...
    def _load_data(self):
        ...
        try:
            ...
            self.esd_offset = self._estimate_esd_offset(
                self.rsd_raw.sum(axis=1), self.esd_traces.sum(axis=1))
    ...
    def _estimate_esd_offset(self, rsd_env, esd_env):
        """The ESD record stream doesn't start at RSD scan 0 - the basecaller
        only starts emitting records some number of scans into the run. Estimate
        that offset by cross-correlating the overall signal envelopes (summed
        across channels) and finding the shift that best aligns the ESD trace
        against a matching stretch of the RSD trace."""
        n_r, n_e = len(rsd_env), len(esd_env)
        if n_e < 10 or n_e >= n_r:
            return 0
        a = rsd_env - rsd_env.mean()
        b = esd_env - esd_env.mean()
        if not np.any(a) or not np.any(b):
            return 0
        corr = sig_correlate(a, b, mode='valid', method='fft')
    ...
    def _update_plot(self):
        ...
        if esd_offset:
            ax3.text(0.01, 0.95, f'ESD offset: +{esd_offset} scans (est.)',
        ...
        if peaks is not None and seq:
            ...
            for p, base in zip(peaks, seq):
                ...
                if p >= n_esd_recs:
                    continue
                p_apex = self._snap_to_peak_apex(self.esd_traces, p, n_esd_recs)
                trace = self.esd_traces[p_apex]
                dom_ch = np.argmax(trace) if np.any(trace > 0) else -1
                if dom_ch < 0:
                    continue
                color = CHAN_COLORS[dom_ch]
                x_disp = p_apex + esd_offset
                idx_rsd = x_disp
                if 0 <= idx_rsd < len(separated) and sep_max[dom_ch] > 0:
                    y_disp = separated[idx_rsd, dom_ch] + 0.05 * sep_max[dom_ch]
    ...
    def _run_ml(self):
        ...
        # Clamp positions to valid range (shifted by the estimated ESD/RSD offset)
        esd_offset = getattr(self, 'esd_offset', 0)
        positions = positions.astype(np.int64) + esd_offset
        n_sep = len(separated)
