...
def _adaptive_noise_channel(sig: np.ndarray, win: int = 401) -> np.ndarray:
    """Per-position noise floor: rolling median-absolute-deviation scaled to a
    Gaussian sigma.  Mirrors the DLL's frequency-/time-adaptive noise estimate
    (ObsInpSpec noise-frac 0.0587) which keeps weak real bands recoverable in
    the tail while not admitting small ripple peaks.  ``win`` is odd."""
    n = len(sig)
    if n == 0:
    ...
    for i in range(n):
        ...
        med[i] = np.median(sig[lo:hi])
    absdev = np.abs(sig - med)
    mad = np.empty(n)
    for i in range(n):
        lo, hi = max(0, i - half), min(n, i + half + 1)
        mad[i] = np.median(absdev[lo:hi])
    return 1.4826 * mad + 1e-6


def pc_dll_peaks(separated: np.ndarray, shifts,
                 region: Optional[Tuple[int, int]] = None,
                 snr: float = 3.0,
                 noise_win: int = 401,
                 min_sig_frac: float = 0.02,
                 min_dist: int = 3,
                 min_width: float = 2.0,
                 merge: int = 3,
                 base_letters: Tuple[str, ...] = DEFAULT_BASE_OF_DYE,
                 ) -> Tuple[np.ndarray, str, List]:
    """Per-channel SNR peak detector + band-width validation + cross-channel
    merge (port of the DLL's Wvfm::putativePks record-list peak detector).

    Unlike the greedy max-intensity caller, which excises +/-GREEDY_WINDOW
    scans around each pick and therefore cannot resolve two real bases closer
    than ~2*GREEDY_WINDOW apart, this detector finds every SNR peak on each
    channel independently (adaptive per-position noise floor) and then merges
    detections that fall within ``merge`` scans into a single base at the
    dominant-channel maximum.  Recovering those tightly-spaced second bases is
    exactly what the earlier greedy caller lost (validated: essentially all of
    the DLL peaks our greedy caller missed have a neighbour within ~15 scans).

    Algorithm (per channel, after mobility shift):
      1. adaptive running-MAD noise floor (``_adaptive_noise_channel``)
      2. find_peaks with height = snr*noise (and a tail-safe amplitude floor)
      3. band-width validation: reject spikes too narrow for the local
         (mobility-lane) position (position-adaptive ``_WIDTH_RATIOS``)
      4. merge channel detections within ``merge`` scans -> one call, dominant
         channel, kept base = base_letters[channel]

    Returns (positions, sequence, intensities) sorted by scan."""
    from scipy.signal import find_peaks
    n = len(separated)
    shifted_all = [dsp_shift_channel(separated[:, ch], int(shifts[ch]))
                   for ch in range(4)]
    sep_max = float(max(separated.max(axis=1).max(), 1e-9)) if n else 1e-9
    start, stop = 0, n
    if region is not None and int(region[1]) > int(region[0]):
        start, stop = max(0, int(region[0])), min(n, int(region[1]))
    ...
    for ch in range(4):
        sig = np.clip(shifted_all[ch], 0.0, None)
        noise = _adaptive_noise_channel(sig, noise_win)
        floor = np.maximum(snr * noise, min_sig_frac * sep_max)
        pk, pr = find_peaks(sig, height=floor, distance=max(1, int(min_dist)),
                            width=min_width)
        heights = pr.get('peak_heights')
        widths = pr.get('peak_widths')
        for i, p in enumerate(pk):
            if not (start <= p < stop):
                continue
            amp = float(heights[i]) if heights is not None else sig[p]
            w = float(widths[i]) if (widths is not None and i < len(widths)) else 2.0
            frac = p / max(n - 1, 1)
            expected_wr = _width_ratio_for_position(frac)   # 1.12 .. 1.94
            # accept bands between ~2/3 and ~3 x the expected width-ratio scale
            if w < 2.0 or w > 3.2 * expected_wr * 6.0:
                continue
            cand.append((int(p), amp, ch))
    ...
    while i < len(cand):
        ...
        cluster = [cand[i]]
        cstart = cand[i][0]
        while j + 1 < len(cand) and cand[j + 1][0] - cstart <= merge:
            j += 1
            cluster.append(cand[j])
        best = max(cluster, key=lambda c: c[1])
        positions.append(best[0])
        letters.append(base_letters[best[2]])
        inten.append(best[1])
        i = j + 1
...
class Cimarron312:
    ...
    def __init__(self, variant: str = "3.12",
                 ...
                 dll_snr: float = 3.0,
                 dll_noise_win: int = 401,
                 dll_min_sig_frac: float = 0.02,
                 dll_min_dist: int = 3,
                 dll_min_width: float = 2.0,
                 dll_merge: int = 3,
        ...
        if caller not in ('greedy', 'cluster', 'fuzzy', 'dll'):
        ...
        self.dll_snr = float(dll_snr)
        self.dll_noise_win = int(dll_noise_win)
        self.dll_min_sig_frac = float(dll_min_sig_frac)
        self.dll_min_dist = int(dll_min_dist)
        self.dll_min_width = float(dll_min_width)
        self.dll_merge = int(dll_merge)
    ...
    def _call_peaks(self, separated: np.ndarray,
        ...
        elif self.caller == 'dll':
            positions, sequence, _ = pc_dll_peaks(
                separated, self.mobility_shifts,
                region=region,
                snr=self.dll_snr,
                noise_win=self.dll_noise_win,
                min_sig_frac=self.dll_min_sig_frac,
                min_dist=self.dll_min_dist,
                min_width=self.dll_min_width,
                merge=self.dll_merge,
